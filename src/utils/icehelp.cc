...
void free_text_node(text_node *n) {
    while (n) {
        text_node *next = n->next;
        delete n;
        n = next;
    }
}
...
void free_data(node *n) {
    while (n) {
        if (n->container)
            free_data(n->container);

        if (n->wrap)
            free_text_node(n->wrap);

        for (int i = 0; i < n->nattr; i++) {
            delete n->attr[i].name;
            delete n->attr[i].value;
        }
        delete [] n->attr;
        ...
        default:
            {
                ...
                do {
                    if (c == '&') {
                        ...
                        do {
                            entity = (char *)realloc(entity, (++elen + 1) | 0xFF);
