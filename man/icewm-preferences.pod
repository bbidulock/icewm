...
The clock time format. See the strftime manpage for the meaning
of all the percent options. It is possible to have multiple
clocks for different time zones by starting each clock format
with a time zone specification in the form of C<TZ=...>,
followed by a space.
