...
workspace it shows the windows which are mapped on it. The bottom
entry reads C<All Workspaces>. It holds the sticky windows. These
windows are mapped in all workspaces.

Click on any workspace to activate it.  Click on a window to focus
it. Or navigate by arrow keys and press Enter.
