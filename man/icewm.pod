...
started before B<icewm>.  See the L<icewmbg(1)> man page for details.
...
A small document browser, which is used by B<icewm> to display the
...
Plays audio files on GUI events which are raised by B<icewm>.
...
=item B<--trace>=I<conf>,I<icon>,I<prog>
...
A single-click on a window entry selects it. Multiple windows can
be selected by C<Shift+Pointer_Button1>. Then a right mouse click will
popup the system menu for this selection. To close the selected windows,
press C<Delete>. Press C<Shift+Delete> to forcefully kill them.

Double-click on a workspace to activate it.  Double-click on a window
to focus it.  Or navigate by arrow keys and press Enter.

The window list window is normally hidden. To activate it, select
the bottom window list menu entry, or press the C<KeySysWindowList>
key, or press the right Windows key if C<Win95Keys=1>.
...
C<KeySysSwitchClass> is like C<KeySysSwitchNext> but only for windows
...
external compositor like L<compton(1)>. If a client window sets the
C<_NET_WM_WINDOW_OPACITY> property on its window then B<icewm> will copy
this to the outer frame window where B<compton> will read it to adjust
the opacity of the client window. The opacity can also be controlled by
B<icewm> when this is configured in the L<icewm-winoptions(5)> file.
Another way is to use L<icewmhint(1)> to preset the opacity level
immediately before starting the application.  The opacity level of
running applications can always be queried or modified by L<icesh(1)>.

The _NET_WM_WINDOW_TYPE properties which B<icewm> sets on its windows
