...
A selected window can be closed by C<Delete> or C<Alt+F4>.
While the C<QuickSwitch> is up, one can still change workspace.
The C<QuickSwitch> also supports mouse operation to activate a window.
