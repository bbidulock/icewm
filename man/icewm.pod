...
L<icewmbg(1)> and B<icewm>.  It also runs the F<startup> script.
...
window manager. This tool is typically used in scripts.

=item L<icehelp(1)>

Is used by B<icewm> to display the 'IceWM manual' and some man pages.

=item L<icewmhint(1)>

A utility for passing IceWM-specific window options to B<icewm>.
The options are used to configure the first application which
is started subsequently.

=item L<icesound(1)>

Plays audio files on GUI events raised by B<icewm>.  It supports ALSA,
AO and OSS.  See the L<icesound(1)> manual page.
