...
or default value. A F<preferences> file is a readable text file which
can be modified with the help of a text editor.  If this option is given
to L<icewm-session(1)> then it is passed on to B<icewm>. If B<icewm> is
started independently then this option can be given to B<icewm>
directly.  However, usually one will want to use a F<preferences> file
