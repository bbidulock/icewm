...
best with a desktop background manager that updates the C<_XROOTPMAP_ID>
property on the root window, such as L<icewmbg(1)>, L<nitrogen(1)>,
L<hsetroot(1)> or L<feh(1)>.
...
Drag and drop is supported in the following ways:

=over

=item

Hovering a drag icon over a task button or a tray icon for
I<AutoRaiseDelay> milliseconds raises the application window.

=item

Hovering a drag icon over a workspace button for at least
I<AutoRaiseDelay> milliseconds activates that workspace.

=item

When edge switching is enabled, bringing the drag icon to the screen
edge for I<EdgeSwitchDelay> milliseconds changes the current workspace.

=item

A hidden taskbar is shown after hovering a drag icon
over its edge for I<AutoShowDelay> milliseconds.

=item

A window list window that has scrollbars will scroll when hovering
a drag icon over the scrollbar scroll arrows.
...
Please report bugs at L<https://github.com/ice-wm/icewm/issues>.
