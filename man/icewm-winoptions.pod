...
For instance, they can control the window border, placement and size,
the window layer, its work space, its visibility on the task bar
...
Here I<NAME> and I<CLASS> are from the B<WM_CLASS> property for the
window. This can be found in the output of C<icesh -a getClass>.

While I<ROLE> refers to the B<WM_WINDOW_ROLE> property for the window,
...
In some rare cases a name, class or role contains a period. If it does,
...
There are four categories of options: I<general>, I<function>, I<decor>
and I<feature>.
...
following names or a number from zero (0) to fifteen (15):
...
the application:
  ...
  ignorePositionHint:       {1|0}  always smart place automatically.
