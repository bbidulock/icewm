...
B<icesh> provides commands for use by shell scripts that query or
affect a window's state. B<icesh> also supports commands for
querying and controlling the L<icewm(1)> window manager.
...
Requests the specified window to be activated, aka. I<to focus>.

=item B<close>

Requests the specified window to be closed.

=item B<kill>

Terminates the specified window.

=item B<list>

Shows details of the specified window, such as geometry and names.
...
Request to raise the specified window.

=item B<above>

Request to stack the window above others.

=item B<below>

Request to stack the window below others.

=item B<rollup>

Request to rollup the specified window.

=item B<fullscreen>

Request to set the window to fullscreen.
...
Request to restore the window to normal.
...
expressions of the domain GNOME window state.  See L</GNOME window
...
Set the GNOME window hints for the specified window to I<HINTS>.  See
...
B<icesh> is somewhat historical. Commands like L<wmctrl(1)> and
L<xdotool(1)> support similar functions and more.
