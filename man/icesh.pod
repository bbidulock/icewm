...
B<icesh> [I<OPTIONS>] I<ACTIONS>

=head1 DESCRIPTION

B<icesh> provides commands for use by shell scripts that affect a
window's state similar to L<wmctl(1)> or L<xdotool(1)>, except more
...
Moves the specified window to another work space.  Select the root
window to change the current work space.

=item B<listWorkspaces>

Lists the names of all work spaces.
...
B<icesh> is historical and deprecated.  The command originally supported
only the GNOME WinWM/WMH specification.  Unlike L<wmctl(1)> and
L<xdotool(1)>, NetWM/EWMH support was never included.
