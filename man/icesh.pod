...
and to interact with the L<icewm(1)> window manager. Multiple commands
can be given. The commands operate on a selection of one or more
windows. B<icesh> has several means to select and filter windows.
...
Command options specify the window or windows to which subsequent
actions apply or filter the set of specified windows. If none is given,
but an action does require a window, then a selection crossbar is
invoked to select the desired window interactively. The manager actions
do not require window options.

The following five options I<select> one or more client windows.
If needed, they can be repeated for multiple actions.
...
If no previous option was given then a B<-all> option is implicitly
...
selected windows. The second directly interacts with the B<icewm> window
manager.
