...
A combination of modifiers and a key. These are separated by a plus-sign,
like in C<Ctrl+Alt+Delete>. The key is either a keystroke name or a key
label. Instead of a key, mouse pointer buttons can be specified by
