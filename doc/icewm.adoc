...
== Window Options (updated 2017-11-15)
...
resources associated with it.  Some more recent applications
may also have a *window role* resource, though not all do.
...
noFocusOnAppRaise=0:: if set to 1, window will not automatically get focus as
application raises it.
ignoreNoFocusHint=0:: if set to 1, icewm will focus even if the window does not
handle input.
...
regular applications. At the moment the window has to be sticky to make it work.
...
startMinimized=0:: if set to 1, window start minimized.
startMaximized=0:: if set to 1, window start maximized.
startMaximizedVert=0:: if set to 1, window start maximized vertically.
startMaximizedHorz=0:: if set to 1, window start maximized horizontally.
