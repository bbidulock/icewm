...
NetworkStatusDevice = "eth0 wlan0":: List of network devices to be displayed in tray, space separated. A special keyword "<sys>" will be expanded to the list of detected interfaces (on Linux) except for "lo".
...
Each window on the desktop should have *class* and *name* resources associated with it. Some more recent applications will also have a *window role* resource, though not all do.
